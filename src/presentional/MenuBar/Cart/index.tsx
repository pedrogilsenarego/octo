...
const Cart = () => {
  ...
  return (
    <Box onWheel={handleWheel} display="flex" flexDirection="column" rowGap={2} padding="2rem">
      ...
      <Divider />
      <Button label={i18n.t("cartDrawer.clearCart")} onClick={() => dispatch(clearCart())} disabled={cartProducts.length <= 0} />
      <Button label={i18n.t("cartDrawer.updateCart")} disabled={cartProducts.length <= 0} />
    </Box>
  );
};
